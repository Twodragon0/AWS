...
# urllib3>=2.2.0 fixes unbounded decompression chain vulnerability
main(['install', '-I', '-q','boto3','requests','opensearch-py==2.4.2', 'urllib3>=2.2.0','--target', '/tmp/', '--no-cache-dir', '--disable-pip-version-check'])
...
# import html  # Used for escaping special characters in the endpoint string
def get_opensearch_client(endpoint):
    service = "aoss" if "aoss" in html.escape(endpoint) else "es"
    logger.debug(f"Connecting to OpenSearch service: {service} at {html.escape(endpoint)}")
    return OpenSearch(
        hosts=[
            {
                "host": endpoint,
