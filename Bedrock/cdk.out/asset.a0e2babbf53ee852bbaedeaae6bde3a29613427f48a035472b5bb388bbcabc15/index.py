...
def handle_message_shortcut(ack, shortcut, client, logger):
    """
    Handle Slack Message Action (callback_id="aws-chatbot") and
    reply with AWS 실행 아키텍처 요약을 Ephemeral 메시지로 전송합니다.
    """
    # 1) 3초 내에 반드시 ack()
    ack()
    ...
    summary = (
        "*📡 AWS Chatbot 실행 요약*\n"
        f"> *선택된 메시지:* `{message_text}`\n\n"
        "- *이벤트 타입:* `message_action` (callback_id=aws-chatbot)\n"
        "- *API Gateway 경로:* `/slack/ask-aws`\n"
        "- *Lambda 함수:* `BedrockKbSlackbotFunction`\n"
        "- *SSM 파라미터:* Slack Bot Token & Signing Secret 조회\n"
        f"- *KnowledgeBase ID:* `{KNOWLEDGEBASE_ID}`\n"
        f"- *RAG Model ARN:* `{RAG_MODEL_ID}`\n"
        "- *Bedrock RetrieveAndGenerate* API 호출\n"
        "- *응답 방식:* `chat_postEphemeral`\n"
    )

    # 4) 사용자에게 Ephemeral 메시지로 전송
