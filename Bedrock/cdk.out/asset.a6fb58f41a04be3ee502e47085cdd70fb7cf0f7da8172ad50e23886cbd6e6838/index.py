...
def handle_message_shortcut(ack, shortcut, client, logger):
    ...
    # 3) Bedrock KB RetrieveAndGenerate 호출 (기존 방식)
    try:
    ...
    # 4) KB에서 응답이 없거나 기본 페일오버 메시지인 경우, invoke_model로 직접 생성
    if not response_text or response_text.lower().startswith("sorry"):
        logger.info("KB 응답 페일오버, invoke_model 폴백 실행")
        try:
            gen_client = boto3.client('bedrock-agent-runtime', region_name=AWS_REGION)
            invoke_resp = gen_client.invoke_model(
                modelId=RAG_MODEL_ID,
                contentType='application/json',
                accept='application/json',
                body=json.dumps({"text": message_text})
            )
            # invoke_model의 body는 JSON 문자열
            body = json.loads(invoke_resp['body'])
            # 결과 배열 첫 번째의 output.text 추출
            response_text = body.get("results", [{}])[0].get("output", {}).get("text", "").strip()
        except Exception as e2:
            logger.error(f"invoke_model 호출 오류: {e2}")
