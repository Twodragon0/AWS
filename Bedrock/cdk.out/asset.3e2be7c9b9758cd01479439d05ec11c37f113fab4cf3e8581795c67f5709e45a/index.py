...
def handle_mentions(event, say, client, logger):
    try:
        ...
        event_id = f"{event.get('event_ts', '')}{salt}"
        
        # 로깅 강화
        logger.info(f"멘션 이벤트 수신: ID={event_id}, user={event.get('user')}")
...
def handler(event, context):
    try:
        # 로깅 강화
        logging.info(f"Lambda 핸들러 호출: event_type={event.get('type', 'unknown')}")
        ...
        logging.info(f"Slack 응답 처리 결과: {response and response.get('statusCode', 'unknown')}")
