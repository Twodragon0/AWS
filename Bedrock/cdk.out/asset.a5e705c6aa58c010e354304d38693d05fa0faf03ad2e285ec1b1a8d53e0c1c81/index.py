...
def handler(event, context):
  ...
  # Handle Slack Shortcut (Message Action)
@app.shortcut("aws-chatbot")
def handle_shortcut(ack, shortcut, client, logger):
    """
    Handle Slack Message Shortcut event with callback_id 'aws-chatbot'.
    """
    try:
        ack()  # Must ack within 3 seconds to prevent timeout message

        # Extract the text from the message user selected
        message_text = shortcut["message"]["text"]
        logging.info(f"Shortcut Triggered - Message Text: {message_text}")

        # Call Bedrock with the selected message
        kb_response = get_bedrock_knowledgebase_response(message_text)
        response_text = kb_response["output"]["text"]

        # Respond directly to user via ephemeral message
        ...
        logger.error(f"Shortcut Error: {err}")
        client.chat_postEphemeral(
            channel=shortcut["channel"]["id"],
            user=shortcut["user"]["id"],
            text=f"🚨 오류가 발생했습니다. 다시 시도해 주세요.\n\nError: `{err}`"
        )
