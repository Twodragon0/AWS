...
def handle_message_shortcut(ack, shortcut, client, logger):
    """
    Handle Slack Message Action (callback_id="aws-chatbot"):
    - 선택된 텍스트를 보여주고,
    - 해당 내용의 원인·분석·해결방법을 Bedrock KB에 물어본 뒤
    - Ephemeral 메시지로 사용자에게 전송합니다.
    """
    # 1) Slack에 3초 내 ACK
    ack()
    ...
    # 3) Bedrock에 보낼 프롬프트 구성: “원인·분석·해결방법” 요청
    prompt = (
        "다음 내용에 대해 원인, 분석, 해결방법을 각각 요약해 주세요:\n\n"
        f"{message_text}"
    )

    # 4) Bedrock 호출
    try:
        kb_response = get_bedrock_knowledgebase_response(prompt)
        response_text = kb_response["output"]["text"]
    except Exception as e:
        logger.error(f"Bedrock 호출 오류: {e}")
        response_text = f"🚨 Bedrock 호출 중 오류가 발생했습니다: {e}"
