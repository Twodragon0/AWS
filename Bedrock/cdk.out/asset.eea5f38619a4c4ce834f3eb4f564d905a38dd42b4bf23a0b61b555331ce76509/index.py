...
def handle_shortcut(ack, body, client):
    try:
        ...
        selected_text = body.get("message", {}).get("text", "")
        
        # Call Bedrock with the selected text
        response = bedrock_client.invoke_model(
            modelId=os.environ["RAG_MODEL_ID"],
            body=json.dumps({
                "prompt": f"Please analyze the following text and provide insights: {selected_text}",
                "max_tokens": 1000
            })
        )
        ...
        answer = response_body.get("completion", "No response generated")
        
        # Send the response back to the user
        client.chat_postMessage(
            channel=body["channel"]["id"],
            text=f"Analysis of the selected text:\n\n{answer}"
        )
        
    except Exception as e:
        logger.error(f"Error handling shortcut: {str(e)}")
        client.chat_postMessage(
            channel=body["channel"]["id"],
